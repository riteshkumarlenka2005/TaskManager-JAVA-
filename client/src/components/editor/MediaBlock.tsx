  ...
  if (!url) {
    return (
      ...
      >
        ...
        <Icon className="w-10 h-10 text-text-secondary mx-auto mb-3" />
        <p className="text-text-secondary text-sm">Click to upload {mediaType}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl">
      {mediaType === 'audio' ? (
        ...
        <video controls src={url} className="w-full rounded-xl border border-white/[0.08]" />
      )}
      <input
        value={caption}
